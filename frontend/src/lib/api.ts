...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    throw new ApiError("Server unavailable. Please check if the backend is running.", 0, null);
  }
  ...
  while (true) {
    ...
    for (const part of parts) {
      ...
      if (eventType === "session") {
        ...
        try { yield { type: "citations", data: JSON.parse(eventData) }; }
        catch { /* skip malformed citations */ }
      } else {
